...
class ThemesService: ServiceProvider {
    ...
    func loadDataFromFiles() {
        if ThemeWordDataManager().getAllWordsCountFromCoreData(theme: data[0].subthemes[0].subthemes[0]) == 0 {
            let csvManager = CSVManager()
            for theme1 in data {
                for theme2 in theme1.subthemes {
                    for theme3 in theme2.subthemes {
                        csvManager.writeWordsFromCSVToCoreData(theme: theme3)
                    }
                }
            }
        }
    ...
    func loadCountData() {
        ...
        for theme1 in data {
            ...
            theme1.countAll = 0
            for theme2 in theme1.subthemes {
                theme2.countArchive = 0
                theme2.countAll = 0
                for theme3 in theme2.subthemes {
                    theme3.countArchive = manager.getArchiveWordsCountFromCoreData(theme: theme3)
                    theme3.countAll = manager.getAllWordsCountFromCoreData(theme: theme3)
                    theme2.countArchive += theme3.countArchive
                    theme2.countAll += theme3.countAll
                }
    ...
    func loadCountData(theme: Theme) {
        if let themes = findThemes(theme) {
            themes.th1.countArchive -= themes.th2.countArchive
            themes.th2.countArchive -= theme.countArchive
    ...
    func findThemes(_ theme: Theme) -> (th1: Theme, th2: Theme)? {
        for th1 in data {
            for th2 in th1.subthemes {
                for th3 in th2.subthemes {
                    if th3.name == theme.name {
                        return (th1: th1, th2: th2)
                    }
                }
            }
        }

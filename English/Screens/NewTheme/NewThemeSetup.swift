...
extension NewThemeView {
    ...
    func setupViews() {
        ...
        toBottomButton.tapHandler = didTapToBottomButton
        ...
        addConstraintsWithFormat(format: "H:[v0]-\(Screen.sideInset)-|", views: toBottomButton)
        addConstraintsWithFormat(format: "V:[v0]-\(Screen.sideInset)-|", views: toBottomButton)
    ...
    func didTapToBottomButton() {
        toBottomButton.isHidden = true
        let items = vm.data
        var lastIndex = -1
        for index in items.indices {
            if items[index].data.isArchive {
                lastIndex = index
            }
        }
        
    
        scrollView.contentOffset.y = tv.frame.origin.y + CGFloat(80 * lastIndex)
    }
